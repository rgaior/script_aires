...
class Detector:
    def __init__(self, temp=100, gain=1e6, f1=0.95e9,f2=1.750e9, tau=5e-9,type=''):
        ...
        if type=='norsat' or type == 'dmx' or type=='helix' or type=='':
            self.capaornot = 0
    ...
    def setpnoise(self):
        if len(self.noisespectrum) != 0:
            ...
            self.deltaB = self.f2 - self.f1
            self.pnoise = constant.kb*self.temp*(self.f2 - self.f1)

        
    #return [time, amp] for the different stages of the detector
    def producesimwaveform(self, simwf, stage, method=None, fcut=None, tau=None):
        if (fcut != None) and (tau!=None):
            afterpd = self.m3_powerdetsim(simwf,tau)
            afterboard = self.adaptationboard2(afterpd,fcut)
...
#power detector characteristic (P[dBm] vs V_pd[V])
    ...
    def m3_powerdetsim(self, wf,tau=None):
        if tau == None:
            ...
            newwf = utils.m3_powerdetectorsim(wf.time,wf.amp,self.gain,tau,self.m3_slope, self.m3_offset)
...
# (see calib/board_HFcarac.py for the numbers)
    def adaptationboard2(self, wf,fcut=None):
...
#        sigma = constant.boardspecsigma
        ...
        b = constant.boardphaseb
        c = constant.boardphasec
        gainphase = utils.boardphasefunc(fftfreq,a,b,c)
        if self.type=='norsatt':
